...
///
/// **A verbatim duplicate of `KeelCore/UTCDate.swift`.** The two packages share no target on
/// purpose (`docs/ARCHITECTURE.md` §7), so the date math that produces sort keys on this side
/// and consumes them on the other is copied rather than shared, and pinned together by
/// `Fixtures/` and by tests in both suites. Change one, change both.
///
/// Deliberately uses neither `Calendar` nor any `FormatStyle`:
/// - `Calendar` and `TimeZone` are absent from `FoundationEssentials`, which is what this
///   target links on Linux — and Linux is where every counter key is actually built.
/// - `Date.ISO8601FormatStyle` exists here but not reliably in the client copy, which has to
///   survive Skip's Kotlin transpiler. Keeping both copies identical is worth more than using
///   the nicer API on one side.
///
