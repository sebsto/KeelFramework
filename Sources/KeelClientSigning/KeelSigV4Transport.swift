...
/// An `HTTPTransport` that signs outgoing requests with AWS SigV4.
///
/// Use it when the app is deployed behind `KeelAuth.iam()`: wrap the real transport and pass
/// IAM credentials, and every request is signed for API Gateway. It depends only on `KeelCore`
/// and `CryptoKit`, and is Apple/CryptoKit-gated because `CryptoKit` is Apple-platform-only —
/// which is why it lives in its own Apple-platform library rather than in `KeelCore`, keeping
/// `KeelCore` dependency-free and Skip-transpilable.
///
/// ### Conditional signing
///
